...
#     프로그램 -> 입력오류 -> 프로그램 종료(Error)
#     프로그램 -> 입력오류 -> 예외처리 -> 프로그램 종료 X
#   - 데이터베이스 또는 파일시스템 사용할 때는 반드시 예외처리!!

#   ** 예외 종류
#   1. 예측 가능한 예외
#    - 발생 여부를 개발자가 사전에 인지 가능 -> 예외처리
#   2. 예측 불가능한 예외
#    - 서버에서 데이터 받기
#    ex) 스마트폰(카카오톡) -> 00친구와 톡방 -> 카카오 서버 접근
#                           -> 데이터 전달 -> 스마트폰 출력
...
# 가정: 수강신청 -> 종합정보시스템(조대 서버): 최대 2,000명 동시접속
...
from urlib.request import urlopen.HTTPError
